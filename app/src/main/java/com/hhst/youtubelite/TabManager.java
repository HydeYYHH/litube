...
import com.hhst.youtubelite.webview.YoutubeFragment;
...
public class TabManager {
	...
	private final Set<String> navTags = Set.of(UrlUtils.PAGE_HOME, UrlUtils.PAGE_SHORTS, UrlUtils.PAGE_SUBSCRIPTIONS, UrlUtils.PAGE_LIBRARY);
	private final Deque<YoutubeFragment> tabs = new LinkedList<>();
	@Getter
	@Nullable
	private YoutubeFragment tab;

	public TabManager(@NonNull final FragmentActivity activity) {
	...
	private void onTabChanged() {
		...
		if (activity instanceof MainActivity mainActivity) {
			if (UrlUtils.PAGE_WATCH.equals(UrlUtils.getPageClass(url)) && url != null)
				mainActivity.play(url);
			else mainActivity.hidePlayer();
		}
	}
	...
	public void openTab(@NonNull final String url, @Nullable String tag) {
		...
		// Open in current tab
		if (tab != null && tag.equals(tab.getMTag()) && navTags.contains(tag)) {
			if (!url.equals(tab.getUrl())) tab.loadUrl(url);
			return;
		}
		final String homeTag = UrlUtils.PAGE_HOME;
		...
		if (!navTags.contains(tag)) {
			// Open tab directly and ensure home is on the bottom
			final var first = tabs.peekFirst();
			if (first == null || !homeTag.equals(first.getMTag())) {
				final var home = YoutubeFragment.newInstance(HOME_URL, homeTag);
			...
			tabs.clear();
			if (home == null) {
				home = YoutubeFragment.newInstance(HOME_URL, homeTag);
				ft.add(R.id.fragment_container, home, homeTag);
			}
			tabs.offer(home);
